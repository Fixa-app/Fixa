...
export default function OnboardingUploadPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      sessionStorage.setItem("onboarding_quoteNumber", parsedData.quoteNumber ?? "");

      setUploading(false);
      setSuccess(true);
    } catch (err) {
      console.error("Parsing error:", err);
      setError("Het document kon niet worden gelezen. Probeer het opnieuw of vul handmatig in.");
      setUploading(false);
      setSuccess(false);
    }
  };
    ...
    <div className="relative min-h-screen">
      ...
      <div className="mx-auto w-full max-w-2xl space-y-6 p-6 pt-16">
        ...
        <div className="space-y-2">
          <h1 className="font-display text-3xl font-semibold">Upload een offerte</h1>
          <p className="text-base text-muted-foreground">
            We halen automatisch alles eruit wat we nodig hebben om offertes via Fixa te versturen.
          </p>
        </div>
        ...
        >
          ...
          <p className="mt-4 text-xl font-semibold text-foreground">
            {uploading ? "Bezig met verwerken..." : file ? file.name : "Offerte uploaden"}
          </p>
