...
function formatLastQuoted(dateStr: string | null) {
  ...
  return `Gebruikt op ${date.toLocaleDateString("nl-NL", { day: "numeric", month: "long", year: "numeric" })}`;
}
...
export default function NewRequestStep2ClientPage() {
  ...
  async function handleFinish() {
    ...
    if (phone.trim() && !/^\+?[\d\s-]{8,}$/.test(phone)) {
    ...
    return (
      <div className="flex min-h-screen flex-col">
        <div className="flex-1 space-y-6 p-6 mx-auto w-full max-w-2xl pb-32">
          ...
          <h1 className="font-display text-3xl font-bold">Wie is de klant?</h1>
          ...
          <div className="space-y-1">
            ...
            {clients.map((client, i) => (
              <button
                ...
                onClick={() => selectClient(client)}
                className="flex w-full items-center gap-4 rounded-xl px-2 py-3 text-left transition-colors hover:bg-muted/50 active:scale-[0.99]"
                style={{ opacity: 1, animation: `fadeUp 0.2s ease ${i * 150}ms both` }}
              >
                <ClientAvatar name={client.name} />
                <div>
                  <p className="font-bold">{client.name}</p>
